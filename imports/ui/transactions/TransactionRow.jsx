...
export const TransactionRow = (props) => {
  ...
  return (
    <SentryBoundary>
      <Row
        className={`${tx.code ? "tx-info w-40 invalid" : "tx-info w-40"} pa1`}
      >
        <Col xs={12} lg={homepage ? 5 : 7} className="activity">
                ...
                return (
                  <div body key={i}>
                    <Activities
                      ...
                      events={
                        tx.tx_response.logs && tx.tx_response.logs[i]
        ...
        >
          ...
          {tx?.tx?.body?.memo && tx?.tx?.body?.memo != "" ? (
            <span>
              ...
              >
                <PopoverBody>
                  <Markdown markup={tx.tx.body.memo} />
                </PopoverBody>
              </UncontrolledPopover>
            </span>
          ) : (
              ...
              tx?.tx?.auth_info?.fee?.amount.map((fee, i) => {
                return (
                  <span className="text-nowrap" key={i}>
                    {new Coin(fee.amount, fee ? fee.denom : null).toString()}
                  </span>
                );
              })
            ) : (
